...
def run_inference(text: str, prev_messages: list = None) -> dict:
    ...
    if onnx_session:
        ...
        topic_probs = (1 / (1 + np.exp(-onnx_out[0][0]))).tolist()
...
def full_pipeline(text: str, room_id: str = None, context: list = None,
    ...
    # Skip lifecycle when state machine already decided (fire/cancel/reminder) —
    # the conversation state machine is authoritative for managed intents.
    sm_action = sm_result.get("action", "")
    sm_decided = sm_action in (Action.FIRE, Action.CANCEL, Action.REMINDER)
    if room_id:
        if not sm_decided:
